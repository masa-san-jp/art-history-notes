...
| 項目 | 内容 |
|---|---|
| 作者 | Georges Seurat（フランス、1859–1891） |
| 制作 | 1884–86年、額縁部分の彩色は1888–89年に追加 |
| 素材 | 油彩・カンヴァス |
| 寸法 | 207.5 × 308.1 cm |
...
> L'art c'est l'Harmonie. L'Harmonie c'est l'analogie des contraires, l'analogie des semblables,
> de ton, de teinte, de ligne, considérés par la dominante et sous l'influence d'un éclairage
> en combinaisons gaies, calmes ou tristes.

（芸術とは調和である。調和とは、対立するものの類似であり、似たものの類似である。明暗・色相・線に
ついてそれを見る。支配的なものを基準に、そして光の影響のもとで、快活な／穏やかな／悲しい組み合わせとして。）

出典: [fr.wikiquote — Georges Seurat](https://fr.wikiquote.org/wiki/Georges_Seurat)（John Rewald『Seurat』
Flammarion, 1990, p.165 からの引用として掲載）。同じ一節の異なる転記が
...
- 点描は「混ぜない色を隣に置く」手。混色して1つにするのではなく、**別のまま隣に置いて、
  見る側の目で1つにする**。調和を絵具の上ではなく鑑賞者の網膜で起こしている。
...
1. **調和を「揃える」で作らない。** 揃えると弱くなる。まず明確に対立するものを2つ置き、
   その2つを同じ規則で扱う。規則の共有が調和で、見た目の一致は調和ではない。
2. **支配（dominante）を先に決める。** 生成に入る前に「この作品は何が支配しているか」を1行で書く。
   支配色なのか、線の向きなのか、明度なのか。決めてから差を作る。決めずに作った画面は必ずばらける。
3. **混ぜずに隣に置く。** 2つの要素を融合させて中間物を作るのではなく、別のまま近接させて、
   見る人の側で像を結ばせる。AI で作るときの具体手としては、1枚に統合するより、
   分離した要素を同一の規則（同じ光源・同じ粒度・同じ画角）で通すこと。

**次にやること**: 上の3つが自分の手で打てるか、テスト1枚で確かめる。テーマ「調和」の
候補案4つのうち、対立物を最初から2つ持っているのはどれかを見る。
