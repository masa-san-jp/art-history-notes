...
**探した語は記録されます**（`data/queries.jsonl`）。該当なしだった語は被覆マップに「探されたが無かった語」
として出て、私が次に何を埋めるかの判断材料になります。つまり**空振りも無駄になりません**——
...
**入れ終わったら issue にコメントして閉じます**（何を入れたか・status がどこまで上がったか・
何が未確認のままか）。頼んだ側が結果を取りに来なくて済むように、こちらから返します。
