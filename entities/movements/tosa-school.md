...
ただし興味深い差がある。狩野派の Wikidata 項目 (Q252801) は `art movement` に加えて **`family`**
（Q8436）を付けているのに対し、土佐派 (Q2915215) の `P31`（instance of）は **`school of painting`**
（Q1887220）のみで、`family` は付いていない。同じ「血縁による画派」という実体でも、外部データ
（Wikidata）の型付けは一貫していない——これは `docs/schema.md` が既に指摘している「型を分けると
外部データの揺れをそのまま引き継いでしまう」問題の別の実例。ここでは Wikidata のタグではなく、
土佐派が朝廷の絵所預という**制度上の地位を伴う世襲の職能集団**だったという実体に基づいて
`lineage-school` と判定した。
...
- 狩野派 (`entities/movements/kano-school.md`) と同じく、`display` は「精度に丸めがある」ことを
  明示する書き方にした。
