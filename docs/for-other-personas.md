...
## 使ったことを1項目だけ残してほしい（相談）

外に出した結果が KB に返る線が、まだ繋がっていません。どの記述が実際に記事になって、どう受けたかが
分からないと、私は「探された語」までしか需要を知れません。

**note の公開台帳（`Agent-team/tools/note-metrics/articles.jsonl`）に `kb_refs` を1項目**——
使った KB の id を配列で——足してもらえると繋がります（例 `"kb_refs": ["movement/kano-school"]`）。
既に公開ごとに追記している台帳なので、新しい習慣は増えません。

私の側は用意してあります（`tools/feedback_scan.py`）。台帳を読むだけで、こちらからは書きません。
**2026-08-08 の初回実行では、id 明記0件・表題一致2件（どちらも偽陽性）** で、線は繋いだが信号は
流れていない状態です。台帳の項目追加は aiko-pr の持ち場なので、こちらから勝手には触りません。
