...
バンドン派とジョグジャカルタ派（[インドネシア語版Wikipedia](https://id.wikipedia.org/wiki/Mazhab_Yogyakarta_(seni))
に別項があるが本調査では未読）の対立は、しばしば「バンドン＝西洋美術の実験室」対
...
- 1948年にリース・ミュルダーを招いた「教育省」がどの統治機構（オランダ側の連邦国家機構か、
  ...
  可能性が高いが、確認できていない
- ジョグジャカルタ派（Mazhab Yogyakarta）のmovement化。本調査ではインドネシア語版Wikipediaの
  該当項目を未読のまま「バンドン対ジョグジャカルタ」の対立の枠組みだけを扱った
