---
id: movement/arts-and-crafts
uri: urn:ahn:movement/arts-and-crafts
type: movement
kind: retrospective
label_ja: アーツ・アンド・クラフツ運動
label_en: Arts and Crafts movement
authority:
  wikidata: Q330369
  aat: "300266319"
  ndl: null
  jpsearch: null
  none_reason: null
time:
  start: "1880"
  end: "1910"
  display: "1880年代（英国で運動の語と結社の枠組みが成立）〜1910年代（英国で中心的な勢いが衰える。海外受容は継続）"
naming:
  self_identified: false
  named_by: null
  named_when: "1887"
  original_label: "Arts and Crafts"
  note: "単一の宣言集団が名乗った名称ではなく、英国の複数の結社・工房・作り手を後からまとめる国際的な運動名。V&Aは、1887年にロンドンで設立されたArts and Crafts Exhibition Societyから運動が名を取ったと説明する"
claims:
  - {field: time, source: "https://www.vam.ac.uk/articles/arts-and-crafts-an-introduction", certainty: scholarly}
  - {field: originated_in, source: "https://www.vam.ac.uk/articles/arts-and-crafts-an-introduction", certainty: scholarly}
  - {field: kind, source: "https://www.vam.ac.uk/articles/arts-and-crafts-an-introduction", certainty: scholarly}
space:
  - {role: originated_in, target: place/london}
relations:
  - {type: diffused_to, target: place/vienna, certainty: scholarly, source: "https://www.mak.at/en/program/exhibitions/vienna_1900"}
sources:
  - https://www.wikidata.org/wiki/Q330369
  - https://www.vam.ac.uk/articles/arts-and-crafts-an-introduction
  - https://www.vam.ac.uk/articles/arts-and-crafts-beyond-the-uk
  - https://www.metmuseum.org/it/essays/design-reform
  - https://www.mak.at/en/program/exhibitions/vienna_1900
status: draft
updated: 2026-08-13
---

# アーツ・アンド・クラフツ運動 / Arts and Crafts movement

19世紀後半の英国で、工業化によって生産と生活から切り離された手仕事・装飾芸術を再評価し、建築・家具・
織物・金工などの設計と製作を結び直そうとした国際的な運動。V&Aは、運動が特定の様式を定めたものという
よりも理念によって組織され、英国で形成された後、欧州・米国・日本へ広がったと整理する。

## 定義と範囲

このエントリは、ウィリアム・モリス個人や特定の工房ではなく、英国の複数の結社・工房・展示活動をまとめる
運動としてのアーツ・アンド・クラフツを対象にする。中心的な問題は、機械化・分業による生産が作り手と
製品の関係を損なうという批判と、手仕事・素材・日用品の美的価値を回復する試みだった。

V&Aによれば、運動は1887年にロンドンで設立されたArts and Crafts Exhibition Societyから名称を取り、
1888年に最初の年次展を開いた。ただし、運動全体はこの一組織と同一ではなく、Arts Workers Guildや
各地の工房・展示団体を含む複数の実践から成る。

## 時間と空間

始点は、英国でこの運動が形成された1880年代を代表して1880年とする。終点は、英国で中心的な勢いが
衰え始めた1910年代を代表して1910年とする。これは手仕事の理念やデザインが1910年以後に消えたことを
意味しない。V&Aが示すように、運動の考えは欧州・米国へ伝わり、1920年代半ばには日本の民藝運動の形成も
助けた。MAKは、ウィーン工房が英国のアーツ・アンド・クラフツ運動をモデルに1903年に設立されたと説明しており、
ウィーンをこの理念が具体的な工芸協同組合へ受容された都市として `diffused_to` に記録する。

ロンドンは運動全体の唯一の発生地ではないが、1887年の展示協会設立と1888年の最初の年次展が行われた
代表的な形成地として `originated_in` に置く。英国各地の都市・農村工房は、個別の団体と活動地を調査してから
追加する。

## 未着手

- ウィリアム・モリス、ジョン・ラスキン、A. W. N. ピュージンらの人物化と、思想・作品を介した関係の分解
- Arts and Crafts Exhibition Society、Arts Workers Guild、Morris & Co. などの組織化
- アール・ヌーヴォーへの具体的な伝播経路
- 英国各地、欧州、米国、日本での地域別展開。特に民藝運動への形成的接続は民藝側に記録済みだが、人物・
  出版物・工芸作品を介した経路の分解は未着手
