...
`1500~`（およそ1500年）/ `1884?`（不確か）/ `-0899`（紀元前）/ `-08XX` / `..`（開いた端）/
`null`（不明）。

#### 紀元前は符号付き年で書く。**年番号は天文学的で、1年ずれる**

ISO 8601-2 に従い、`0000` が**紀元前1年**、`-0001` が紀元前2年、`-0899` が**紀元前900年**を指す。
「紀元前900年だから -0900」は誤り。**この1年のずれは必ず事故になるので、原表記（「紀元前900年頃」
「500 B.C.」など出典が書いたまま）を `time.display` に残す**（決定: [issue #1](https://github.com/masa-san-jp/art-history-notes/issues/1)、
2026-08-13。それまで `tools/kb.py` の正規表現が先頭のマイナスを弾いており、先史・古代が丸ごと
時間軸から落ちていた）。

マスクの展開は符号で向きが逆になる——`09XX` は 900〜999 だが `-09XX` は -999〜-900 で、最小と最大が
入れ替わる。被覆マップの世紀キーは紀元前を `-10`（前10世紀）のように符号付きで分ける。
実装は `tools/kb.py` の `edtf_year_range` / `century_label`、退行を止めるテストは
`tools/test_edtf.py`（`python3 tools/test_edtf.py`）。

**表現できることと、書いてよいことは別である。** 出典が単一の年に収束していないなら `null` の
ままにする——[ノク彫刻](../entities/movements/nok-sculpture.md)は、彫刻の出現を紀元前900年とする
再調査と紀元前500年とする古い推定に割れており世紀をまたぐため、`start` を `null` に残して幅を
`display` に書いた。器が直ったことは、推測で埋めてよい理由にならない。
