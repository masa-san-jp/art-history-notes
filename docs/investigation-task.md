...
| `retrospective` | **外部**（批評家・史家・市場・美術館・行政）が付けた括り。**後代とは限らない**——同時代に名づけられていても、名づけたのが当事者でなければこちら（日本画は成立と同じ1882年にフェノロサが命名、金陵八家は当事者の多くが存命中の1663年。判定の詳細と実例表は `docs/schema.md` の kind 節） | 印象派、ポスト印象派、マニエリスム、日本画 |
...
- **ウィキペディアのURLを手で組み立てない。** 漢字・キリル文字の記事URLを読みから
  percent-encoding で書き起こすと、1字違いで実在しないURLができる（2026-08-10 実測: 王鑑を
  王鑒と取り違えて404を作った。引用文は本物、URLだけ偽物という状態になる）。次で解決させる:

  ```bash
  python3 tools/wikiurl.py zh "王鑑 (畫家)"   # repo 内に既出ならそれを、無ければ API に解決させる
  ```
