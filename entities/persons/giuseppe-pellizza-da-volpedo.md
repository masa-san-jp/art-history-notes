---
id: person/giuseppe-pellizza-da-volpedo
uri: urn:ahn:person/giuseppe-pellizza-da-volpedo
type: person
label_ja: ジュゼッペ・ペリッツァ・ダ・ヴォルペード
label_en: Giuseppe Pellizza da Volpedo
aliases:
  - person/giuseppe-pellizza
authority:
  wikidata: Q1528580
  ulan: "500028941"
  ndl: null
  jpsearch: null
  none_reason: null
time:
  start: "1868-07-28"
  end: "1907-06-14"
  display: "1868年7月28日ヴォルペード生まれ — 1907年6月14日同地没"
space: []
relations:
  - {type: belongs_to, target: movement/italian-divisionism}
  - {type: exhibited_at, target: event/first-brera-triennale-1891}
sources:
  - url: "https://www.wikidata.org/wiki/Q1528580"
    kind: authority
  - url: "https://www.treccani.it/enciclopedia/pellizza-da-volpedo-giuseppe_%28Dizionario-Biografico%29/"
    kind: reference
  - url: "https://www.pellizza.it/biografia/"
    kind: reference
  - url: "https://commons.wikimedia.org/wiki/File%3APrima_esposizione_triennale_1891_-_catalogo_ufficiale_illustrato_%28IA_primaesposizione00trie%29.pdf"
    kind: scholarly
status: draft
updated: 2026-08-13
---

# ジュゼッペ・ペリッツァ・ダ・ヴォルペード / Giuseppe Pellizza da Volpedo

ヴォルペードに生まれ、同地で没した画家。後年の通称は「Pellizza da Volpedo」だが、1891年の公式カタログでは
「Pellizza Pippo di Volpedo」と記載される。この表記差は同一人物を指すものとして、別人ノードを作らずエイリアスで保持する。

## 1891年第一回ブレラ三年展

公式カタログ本文は、ペリッツァを次の番号で掲載する。確認できた3件は、それぞれ
[`《Ritratto di Giani Giuseppe mediatore》`](../works/ritratto-di-giani-giuseppe-pellizza-1891.md)、
[`《Pensieri》`](../works/pensieri-pellizza-1891.md)、
[`《Ritratto di mia mamma》`](../works/ritratto-di-mia-mamma-pellizza-1891.md)
として作品記録に分離した。

父の肖像[`《Ritratto di mio papà》`](../works/ritratto-di-mio-papa-pellizza-1891.md)も、作者団体資料が1891年展への出品を確認している。

- 384《Ritratto di Giani Giuseppe mediatore》
- 440《Pensieri》
- 557《Ritratto di mia mamma》

一方、ペリッツァ協会とMuseoTorinoの伝記は、1891年展の出品作として両親の肖像を挙げ、MuseoTorinoは
526・531（両親の肖像）、610《Il mediatore (Ritratto di Giuseppe Giani)》、608《Pensieri (Teresa)》という番号を付している。
同ページは、この番号をScottiの総合カタログ（1986）の作品番号として説明しており、1891年展の公式カタログ番号とは別の体系である。
実際、公式PDFの526番はPizzi Carloの《La bambola rotta》、531番はPoma Silvioの《Novembre》であり、ペリッツァの父母肖像には対応しない。
したがって、本項では公式PDFの展覧会番号384・440・557と、総合カタログ番号526・531・608・610を併記するが、番号を相互に置き換えない。
ペリッツァ協会が1891年出品作として挙げる父の肖像は、作品エンティティ化したが、公式PDF本文で作品名・番号を特定できていない。

## 未着手

- 父の肖像に対応する公式展覧会カタログ番号を確定すること（総合カタログ番号526を公式PDFの番号として流用しない）
- 1894年第二回ブレラ三年展の分割主義作品と比較すること
